...
export default {
  ...
  methods: {
    ...
    init() {
      this.$store
        .dispatch("GET_CHECKLIST_CERT")
        .then(result => {
          this.checklist = this.$store.state.checklist_cert_tables.checklist;
    ...
    submit() {
      if (this.validate()) {
        this.$store
          .dispatch("ADD_CHECKLIST_CERT", this.new_checklist)
          .then(result => {
    ...
    save() {
      if (this.validate()) {
        this.$store
          .dispatch("EDIT_CHECKLIST_CERT", this.new_checklist)
          .then(result => {
