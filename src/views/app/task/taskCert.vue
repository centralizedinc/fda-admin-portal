...
export default {
  ...
  methods: {
    ...
    init() {
      this.$store
        .dispatch("GET_TASK_CERT")
        .then(result => {
          this.tasks = this.$store.state.task_cert_tables.tasks;
...
//View Items
    ...
    submit() {
      if (this.validate()) {
        this.$store.dispatch("ADD_TASK_CERT", this.new_task).then(result => {
          console.log("added:taskCert: " + JSON.stringify(result));
          this.init();
          this.$notify({
    ...
    save() {
      if (this.validate()) {
        this.$store.dispatch("EDIT_TASK_CERT", this.new_task).then(result => {
