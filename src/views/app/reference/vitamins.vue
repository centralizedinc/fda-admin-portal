<template>
  <div>
          ...
          <v-card-text>
            <v-container grid-list-xl>
              <v-layout row wrap align-center justify-center fill-height>
                ...
                <v-flex xs6>
                  <label class="subheading">{{getAdmin(new_vitamins.created_by).first_name}}</label>
                </v-flex>
                <v-flex xs6>
                  <label class="title">Date Created:</label>
                </v-flex>
                <v-flex xs6>
                  <label class="subheading">{{formatDate(new_vitamins.date_created)}}</label>
                </v-flex>
                <v-flex xs6>
                  <label class="title">Modified By:</label>
                </v-flex>
                <v-flex xs6>
                  <label class="subheading">{{getAdmin(new_vitamins.modified_by)}}</label>
                </v-flex>
                <v-flex xs6>
                  <label class="title">Date Modified:</label>
                </v-flex>
                <v-flex xs6>
                  <label class="subheading">{{formatDate(new_vitamins.date_modified)}}</label>
                </v-flex>
              </v-layout>
            </v-container>
          </v-card-text>
    ...
    <!-- TABLE -->
    <v-data-table :headers="headers" :items="vitamins" :search="search" class="elevation-1">
...
export default {
  ...
  methods: {
    ...
    addItem() {
      ...
      this.mode = 0; // Create
      this.new_vitamins = {}; // holds the filled up item
      this.dialog = true;
    },
    editItem(item, index) {
      this.mode = 1; // Edit
      this.selectedIndex = index;
      this.new_vitamins = JSON.parse(JSON.stringify(item));
      this.dialog = true;
    },

    viewItem(item) {
      this.new_vitamins = item;
    ...
    validate() {
      var check = true;
      if (
        this.isEmpty(this.new_vitamins.name)
      ) {
        this.$notify({
        ...
        for (let i = 0; i < this.vitamins.length; i++) {
          if (
            this.selectedIndex != i &&
            this.vitamins[i].name.toLowerCase() ===
            this.new_vitamins.name.toLowerCase()
          ) {
            check = false;
          } else if (!check) {
            this.$notify({
    ...
    submit() {
      if (this.validate()) {
        this.$store.dispatch("ADD_VITAMINS", this.new_vitamins).then(result => {
          console.log("added:Company vitamins " + JSON.stringify(result));
          this.init();
          this.$notify({
            message: "You have successfully created a new type of vitamins",
            icon: "check_circle",
            color: "primary"
          });
          this.close();
    ...
    save() {
      if (this.validate()) {
        this.$store.dispatch("EDIT_VITAMINS", this.new_vitamins).then(result => {
          console.log("edited:Company vitamins: " + JSON.stringify(result));
          this.init();
          this.$notify({
            message: "You have successfully edited a type of vitamins",
            icon: "check_circle",
            color: "primary"
          });
          this.close();
